...
1. Missing Row-Level Security (RLS) — Supabase/Postgres tables without RLS policies, or app-level-only filtering where the database itself does not enforce that user A cannot read user B's data. This applies to WRITES as well as reads: a table with a SELECT policy but no UPDATE/DELETE policy, or UPDATE/DELETE policies without an ownership clause (USING / WITH CHECK auth.uid() = user_id), lets any signed-in user modify or wipe other users' rows without ever being able to read them. This is the #1 cause of vibe-coded app breaches. Flag as CRITICAL — BUT only when the CODE ITSELF shows evidence of it: a table/policy definition created without RLS, or queries that rely solely on client-side filtering. Do NOT flag this just because client code uses a public anon/publishable key — a public key WITH RLS enabled is the correct, secure Supabase pattern, and you cannot see the database configuration from client-side code. Never report "no evidence of RLS" as a finding; you cannot verify backend config you cannot see.
...
4. Broken authentication & access control — missing auth checks, client-side-only authorization, inverted access logic. Check the WRITE path, not just reads: an update/delete handler or query (PATCH, PUT, DELETE, .update(), .delete(), .upsert()) that takes a resource ID from the request and acts on it without confirming the current user owns it is an IDOR, even if reads are protected — changing someone else's row needs only their ID. Flag as CRITICAL.
