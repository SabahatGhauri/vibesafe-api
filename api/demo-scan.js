...
Check access control on WRITES, not just reads: an update/delete (PATCH, PUT, DELETE, .update(), .delete(), .upsert()) that acts on a resource ID from the request without confirming the current user owns it is an IDOR — flag as critical even if reads are protected. Same for Supabase tables whose RLS covers SELECT but has no ownership-checked UPDATE/DELETE policy.
