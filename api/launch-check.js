...
"failed": [{"title":"<issue>","category":"technical|ux","severity":"critical|warning|info","why":"<why it matters to users, 1 sentence>","fix":"<what to do, 1 sentence>"}, ...],
...
Every item in "failed" needs a "category": "technical" for crashes, console errors, failed requests, broken images, slow loads — things that are objectively broken. "ux" for flow/friction issues that are subjective but still real: a page with forms=0 and buttons=0 where the goal implies an action is expected (dead end); a page whose h1/title doesn't match what the clicked link text promised (confusing navigation); a form visible in the screenshot with no visible submit control; a page that looks unstyled or broken relative to the others (inconsistent, looks unfinished); missing navigation back to other key pages. These come from actually looking at the screenshots, not the evidence counts alone. Do not invent friction that isn't visible in the evidence or screenshots, and do not penalize a page for simply being minimal if nothing is actually broken. Be honest but encouraging. The user's stated goal matters — address it.`,
